...
export class BotAI {
  static update(game, bot, dt) {
    ...
    // --- HUNT MODE OVERRIDE (ping mechanic) ---
    if (bot.huntEndTime && performance.now() < bot.huntEndTime && bot.huntTarget) {
      // In hunt mode: ignore everything else, move toward hunt target, shoot at player if in range
      const ht = bot.huntTarget;
      const dx = ht.x - bot.x;
      const dy = ht.y - bot.y;
      const dist = Math.sqrt(dx * dx + dy * dy);
      
      // Stop moving if we've reached the target (within 30px)
      if (dist > 30) {
        const moveX = dx / dist;
        const moveY = dy / dist;
      ...
      // Shoot at player if in range and visible (line of sight)
      ...
      // Clear hunt data if expired
      bot.huntTarget = null;
      bot.huntEndTime = null;
    }

    // --- NORMAL AI (outside hunt) ---
    ...
    // In "THE Target" mode, force player target with probability
    ...
    // If player is closer than current nearest and visible, still pick player
    ...
    if (outsideZone && (bot.hp < 60 || game.zone.shrinking)) {
      ...
      const engageDist = WeaponDefs[wType]?.range * 0.9;
    ...
    switch (ai.state) {
      case 'wander':
        if (game.gameMode !== 'target' || !game.player.alive) {
          ai.wanderTimer -= dt;
          if (ai.wanderTimer <= 0) { ai.wanderAngle = Math.random() * Math.PI * 2; ai.wanderTimer = 2 + Math.random() * 3; }
        }
        moveX = Math.cos(ai.wanderAngle); moveY = Math.sin(ai.wanderAngle); bot.angle = ai.wanderAngle;
        break;
      case 'loot':
        if (ai.target) {
          const dx = ai.target.x - bot.x, dy = ai.target.y - bot.y, d = Math.sqrt(dx * dx + dy * dy);
          if (d > 30) { moveX = dx / d; moveY = dy / d; bot.angle = Math.atan2(dy, dx); }
          else { LootSystem.pickupLoot(bot, game); ai.state = 'wander'; ai.target = null; }
        } else ai.state = 'wander';
        break;
      case 'chase':
        if (ai.target && ai.target.alive) {
          const dx = ai.target.x - bot.x, dy = ai.target.y - bot.y, d = Math.sqrt(dx * dx + dy * dy);
          moveX = dx / d; moveY = dy / d; bot.angle = Math.atan2(dy, dx);
          if (d < WeaponDefs[bot.currentWeaponData.type]?.range * 0.8 && ai.shootCooldown <= 0) {
            CombatSystem.tryShoot(game, bot); ai.shootCooldown = 0.3 / ai.accuracy;
          }
        } else { ai.state = 'wander'; ai.target = null; }
        break;
      case 'engage':
        if (ai.target && ai.target.alive) {
          const dx = ai.target.x - bot.x, dy = ai.target.y - bot.y, d = Math.sqrt(dx * dx + dy * dy);
          ...
          moveX = Math.cos(strafe) * 0.3; moveY = Math.sin(strafe) * 0.3;
          const wType = bot.currentWeaponData.type;
          const ideal = wType === 'shotgun' || wType === 'shotgunPro' ? 80 : wType === 'sniper' ? 350 : 200;
          if (d > ideal + 50) { moveX += dx / d * 0.5; moveY += dy / d * 0.5; }
          else if (d < ideal - 50) { moveX -= dx / d * 0.5; moveY -= dy / d * 0.5; }
          if (ai.shootCooldown <= 0 && !bot.reloading) { CombatSystem.tryShoot(game, bot); ai.shootCooldown = 0.2 / ai.accuracy; }
          if (bot.currentWeaponData?.ammo === 0 && !bot.reloading) WeaponActions.startReload(game, bot);
        } else { ai.state = 'wander'; ai.target = null; }
        break;
      case 'flee_zone':
        const zdx = game.zone.nextCx - bot.x, zdy = game.zone.nextCy - bot.y, zd = Math.sqrt(zdx * zdx + zdy * zdy);
        if (zd > 20) { moveX = zdx / zd; moveY = zdy / zd; bot.angle = Math.atan2(zdy, zdx); }
  ...
  static autoPickupBot(game, bot) {
    // Skip looting if we are in hunt mode
    ...
    for (let i = game.lootItems.length - 1; i >= 0; i--) {
      ...
      if ((bot.x - item.x) ** 2 + (bot.y - item.y) ** 2 < range * range) {
        if (item.type === 'weapon') {
          ...
          } else if (exist) { exist.reserveAmmo += WeaponDefs[item.subtype].ammoMax; game.lootItems.splice(i, 1); }
        } else if (item.type === 'heal') {
          if (item.subtype === 'bandage') bot.bandages += 2; else bot.medkits += 1;
          game.lootItems.splice(i, 1);
